...
<style>
    ...
    @media (max-width: 768px) {
        ...
        .card-header .col-md-6 { margin-bottom: 1rem; }
        .card-header .text-end { text-align: left !important; }
    }
                            ...
                            <div class="row text-center">
                                <div class="col-4">
                                    <div class="border-end">
                                        <h3 class="h5 mb-1 text-success">{{ $usuarios->where('rol_id', 'administrador')->count() }}</h3>
                                        <small class="text-muted">Administradores</small>
                                    </div>
                                </div>
                                <div class="col-4">
                                    <div class="border-end">
                                        <h3 class="h5 mb-1 text-secondary">{{ $usuarios->where('rol_id', 'entrenador')->count() }}</h3>
                                        <small class="text-muted">Entrenadores</small>
                                    </div>
                                </div>
                                <div class="col-4">
                                    <h3 class="h5 mb-1 text-info">{{ $usuarios->total() }}</h3>
                    ...
                    <div class="d-flex justify-content-end align-items-center gap-3">
                        <div class="d-flex align-items-center gap-2">
                            <div class="input-group" style="max-width: 280px;">
                                ...
                                <input type="text" class="form-control border-start-0" id="searchInput" placeholder="Buscar usuario...">
                            </div>
            ...
            <table class="table table-hover mb-0" id="usuariosTable">
                <thead class="table-light">
                    <tr>
                        <th class="border-0 py-3 px-4">
                            <i class="fas fa-user text-muted me-2"></i>Usuario
                        </th>
                        <th class="border-0 py-3 px-4">
                            <i class="fas fa-envelope text-muted me-2"></i>Email
                        </th>
                        <th class="border-0 py-3 px-4">
                            <i class="fas fa-user-shield text-muted me-2"></i>Rol
                        </th>
                ...
                <tbody>
                    ...
                    <tr class="usuario-row">
                        <td class="py-3 px-4">
                            <div class="d-flex align-items-center">
                                <div class="position-relative me-3">
                                    <img src="{{ $user->profile_photo_url ?? '/images/default-avatar.png' }}" 
                                         alt="Foto de {{ $user->name }}" 
                                         class="rounded-circle border-2 border-light shadow-sm usuario-avatar" 
                                         style="width: 45px; height: 45px; object-fit: cover;">
                                    <div class="position-absolute bottom-0 end-0 bg-{{ $user->rol_id == 'administrador' ? 'success' : 'secondary' }} rounded-circle border-2 border-white" 
                        ...
                        <td class="py-3 px-4">
                            <span class="badge bg-{{ $user->rol_id == 'administrador' ? 'success' : 'secondary' }} text-white px-3 py-2 rounded-pill">
                                <i class="fas fa-{{ $user->rol_id == 'administrador' ? 'crown' : 'user-tie' }} me-1"></i>
                                {{ ucfirst($user->rol_id) }}
                            </span>
                        </td>
                    ...
                    <tr>
                        <td colspan="4" class="text-center py-5">
                            <div class="py-5">
                                ...
                                <p class="text-muted mb-4">Aún no se han registrado usuarios en el sistema.</p>
                                @if(auth()->user()->rol_id=="administrador")
                                <a href="{{ route('usuarios.create') }}" class="btn btn-primary">
...
<script>
    ...
    function confirmDelete(id, nombre) {
        if (confirm(`¿Estás seguro de eliminar el usuario "${nombre}"? Esta acción no se puede deshacer.`)) {
