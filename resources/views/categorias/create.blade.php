                    ...
                    <form action="{{ route('categorias.store') }}" method="POST" enctype="multipart/form-data" class="create-form">
                        ...
                        <div class="form-group mb-5">
                            ...
                            <select name="estatus" id="estatus" class="form-select @error('estatus') is-invalid @enderror" required>
                                <option value="{{ \App\Models\Categorias::ESTATUS_ACTIVO }}" {{ old('estatus', \App\Models\Categorias::ESTATUS_ACTIVO) === \App\Models\Categorias::ESTATUS_ACTIVO ? 'selected' : '' }}>
                                    {{ __('Activo') }}
                                </option>
                                <option value="{{ \App\Models\Categorias::ESTATUS_BLOQUEADO }}" {{ old('estatus') === \App\Models\Categorias::ESTATUS_BLOQUEADO ? 'selected' : '' }}>
                                    {{ __('Bloqueado') }}
                                </option>
                            </select>
